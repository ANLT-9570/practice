...
<body class="easyui-layout">
  ...
  //选择 查看工程触发
    ...
    function init(){
		...
		$.ajax({
			async:false,
			url:'${pageContext.request.contextPath }/takWorking/taskTab',
			dataType:'json',
  ...
  //muniLength=1和coutyLength大于1说明是市管理员
    ...
    if(zllbCode != undefined){
    	...
    	var url = encodeURI('${pageContext.request.contextPath}/takWorking/epcTaskData?year='+year+'&disCode='+disCode+'&usr='+usr+'&zllb='+taskMark+'&stat='+preceedStat);
    	$.ajaxSettings.async=false;
    	$("#tab").datagrid({
    		fit:true,
			url:url,
    ...
    function overs(audits,submits,backs){
    	...
    	if(i==0){
			...
			var urls = encodeURI('${pageContext.request.contextPath}/takWorking/proceed?zllbs='+array+'&county='+county+'&year='+year);
